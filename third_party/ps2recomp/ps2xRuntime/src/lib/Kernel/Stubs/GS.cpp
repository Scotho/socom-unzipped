...
namespace ps2_stubs
{
    ...
    void sceGsResetGraph(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        if (mode == 0)
        {
            ...
            GsGParam &gparam = gsRuntimeStateFor(runtime).gparam;
    ...
    void sceGsSetDefDBuffDc(uint8_t *rdram, R5900Context *ctx, PS2Runtime *runtime)
    {
        ...
        const uint64_t smode2 =
            (static_cast<uint64_t>(gsRuntimeStateFor(runtime).gparam.interlace & 0x1u) << 0) |
            (static_cast<uint64_t>(gsRuntimeStateFor(runtime).gparam.ffmode & 0x1u) << 1);
