...
`scripts/make_portable.sh` on Linux (run in the socom-linux VM, which builds the release tarball) copies into
`socom2-linux/lib/` the shared libraries the binaries load, walked from the executables through `DT_NEEDED` and
stopped at the host's own stack (`scripts/portable_libs.py`: glibc, the compiler runtime, GL, X11/Wayland,
ALSA/PulseAudio, VA-API/VDPAU/DRM stay the player's). The release link uses `-Wl,--as-needed`, so the runner needs
only libavcodec, libavutil and libswscale; libavformat and what only it loads are not carried. Those libraries are
the VM's Ubuntu 24.04 builds (the FFmpeg family at 7:6.1.1-3ubuntu5 on 2026-09-25). Ubuntu builds FFmpeg with
`--enable-gpl` and links x264, x265 and Xvid, so the libav* in this tarball are GPL-2.0-or-later as combined (the
project's own GPL-3.0 is compatible). The rows are per Ubuntu source package and name only the libraries the
tarball carries; the licence is the library's as the package's `debian/copyright` states it, in SPDX terms. The
list is `tools_py/tests/fixtures/linux_tarball_lib.txt` (its header says how it was read: 76 libraries, the count the
last measured tarball had); the notices test checks it both ways, and against a real tarball when one is present.
...
| bzip2 | Ubuntu 24.04 `bzip2` | 1.0.8-5.1ubuntu0.1 | bzip2-1.0.6 | 1996-2019 Julian R Seward | yes (`libbz2.so.1.0`) |
...
| Expat | Ubuntu 24.04 `expat` | 2.6.1-2ubuntu0.6 | MIT | Thai Open Source Software Center Ltd, Clark Cooper and the Expat maintainers | yes (`libexpat.so.1`) |
| FFmpeg (Ubuntu's build, configured --enable-gpl) | Ubuntu 24.04 `ffmpeg` | 7:6.1.1-3ubuntu5 | LGPL-2.1-or-later AND GPL-2.0-or-later | the FFmpeg developers | yes (`libavcodec.so.60`, `libavutil.so.58`, `libswresample.so.4`, `libswscale.so.7`) |
...
| GLib | Ubuntu 24.04 `glib2.0` | 2.80.0-6ubuntu3.9 | LGPL-2.1-or-later | the GLib authors (the GNOME Project) | yes (`libgio-2.0.so.0`, `libglib-2.0.so.0`, `libgmodule-2.0.so.0`, `libgobject-2.0.so.0`) |
...
| Highway | Ubuntu 24.04 `highway` | 1.0.7-8.1build1 | Apache-2.0 | Google LLC | yes (`libhwy.so.1`) |
| ICU | Ubuntu 24.04 `icu` | 74.2-1ubuntu3.1 | Unicode-3.0 | Unicode, Inc. and others | yes (`libicudata.so.74`, `libicuuc.so.74`) |
...
| libtheora | Ubuntu 24.04 `libtheora` | 1.1.1+dfsg.1-16.1build3 | BSD-3-Clause | the Xiph.Org Foundation | yes (`libtheoradec.so.1`, `libtheoraenc.so.1`) |
| libvorbis (vorbis, vorbisenc, vorbisfile) | Ubuntu 24.04 `libvorbis` | 1.3.7-1build3 | BSD-3-Clause | the Xiph.Org Foundation | yes (`libvorbis.so.0`, `libvorbisenc.so.2`) |
...
| ZVBI | Ubuntu 24.04 `zvbi` | 0.2.42-2 | LGPL-2.0-or-later | Michael H. Schimek and the ZVBI authors | yes (`libzvbi.so.0`) |
