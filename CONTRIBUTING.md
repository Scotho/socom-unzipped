...
| Your own r0001 disc as well | the game -- **once you have produced the decrypted overlays and the extracted disc tree, which is not yet a documented, runnable step (see below)** | `./build.sh recomp`, `./build.sh runtime`, `./build.sh test` (Windows, Git Bash) or `scripts/build_linux.sh`; see `docs/DEVELOPING.md` "From your own disc to a buildable ELF". |

**The honest state of the game build (2026-09-21).** `./build.sh recomp` starts from files this repository does not and must not contain: the extracted disc tree (`game/disc/`) and the plaintext overlays merged into one ELF (`game/overlays/socom2_game.elf`). The tooling that produced them exists here (`tools_py/decrypt_apache.py` drives the game's own loader under Unicorn to decrypt `RUN/RAW/APACHE00.ZDB`; `tools_py/dnas_selfdecrypt.py`, `tools_py/iso_lbn.py`, `tools_py/make_overlay_elf.py`), but it was run once, by hand, on the maintainer's machine, and the sequence is not written down as a recipe or proven on any other. **So today a contributor can build and test everything except the game.** Making that step reproducible is the next developer-facing job; if you want to attempt it, open an issue and say what you have.
