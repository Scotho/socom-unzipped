...
| Rendering through an OpenGL backend with an integer up-scale (`PS2X_GS_SCALE` 1-4; 3-4 are untested); a CPU rasteriser for tests | Frame rate below the console's 60: the parity gate's mission stage read `FRAME mean=28.37 worst=41.67 n=5609` (host ms per guest VBlank after the HUD, about 35 frames a second) on 2026-09-25 in `s13_proof4_gate`, and the day's two earlier gates read means of 23.22 (`s13_proof2_gate2`) and 35.84 (`s13_proof3_gate`), a spread too wide to set a bar on yet (S13-R3; Sprint 13 Task V4). |
