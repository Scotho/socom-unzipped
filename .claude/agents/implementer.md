...
- Commit only the files the brief names, by explicit pathspec (`git add -- <paths>`); never `git add -A` or `git add .`.
- Subjects `type(scope): what and why` under 120 characters; end the message with the two trailer lines the brief gives.
