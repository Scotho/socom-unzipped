...
**Where the line goes.** `SendMediusChat` sends a Medius chat message; our Horizon (`server/horizon-server`,
...
because the wrapped callback is not on the path a lobby chat line takes through our Horizon. **Ruling O2-R2
...
- #26's bar as written cannot be met by any run: it names a line from a function a lobby chat line does not pass
  through. Before a harness run can prove the receive bound, the receive function has to be found (a trace of
