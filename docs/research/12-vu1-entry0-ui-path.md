...
| qwords **40 … 40+3N-1** | **the staging array**: 3 qwords per vertex, laid out in the GIFtag's `REGS` order — `[0]` = **ST** (perspective-correct: `0x0df8` stores `uv * 1/w` there. The `Q` the PACKED ST format uses is the **`.z`** lane; the `.w` lane of that qword is vf31's clip `w`, which PACKED ST ignores), `[1]` = **RGBAQ** (written by `0x0df8` from the source colour, overwritten wholesale by the template fill `0x54` from qword 327, scaled by lighting `0x18`), `[2]` = **XYZF2** (screen XYZ from the perspective divide; its `.w` is the **F** fog field, and that is the single lane command `0x10` writes) | **[verified]** — `0x1780` loads `40(vin)`→REG0, `41(vin)`→REG1, `42(vin)`→REG2 against `REGS = ST, RGBAQ, XYZF2`; `0x0df8` stores `vf31/vf24/vf26` to `+0/+1/+2`; `0x05d8` stores to `+1` of every vertex; `0x0f90`'s `SQ.w` targets resolve to qwords 42 and 45, i.e. `+2`. The poison sweep confirms the extent: qwords **40-243** (68 vertices × 3 = 204) plus the fill-loop overshoot at **245** |
