...
> **Note (2026-09-24, Sprint 12, S12-R5 in `docs/superpowers/plans/2026-09-24-sprint-12.md`):** the 81.1 % is not reproducible from tracked inputs. The first three passes reproduce commit `e92691a`'s counts on the r0004 map of `a9707ce`; the remaining 2,733 placements came from 285 `--seed` pairs that were never committed. `tools_py/addresses_from_match.py`'s documented recipe (the identity seed alone) gives `resolved 10008/14879 = 67.26%` on today's tracked map. Sprint 12's Task 5 (2026-09-24, `tools_py/derive_seeds.py`, `recomp/r0004_seeds.txt`: 294 seeds, one per distinct offset among the `exact` placements) makes it reproducible: `resolved 12136/14879 = 81.56%` (exact 6,226, hash+callees 257, relinked-body 2,800, seed+delta 2,853, unresolved 2,743) by the two-pass recipe in `tools_py/addresses_from_match.py`'s docstring. The rows below that cite `match.json` were computed from the 81.1 % run and stand as that run's facts.
