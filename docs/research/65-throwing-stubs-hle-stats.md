...
name=...` to stderr on every one of its first calls, before it throws, and the gate's launcher (`tools_py/parity/drive.py`, `stderr=subprocess.STDOUT`) merges stderr into the stage log.
