...
[ "$1" = "harness-only" ] || bash scripts/loop_lock.sh run agent-upstream --purpose "item1 spike: PR244 ps2xIOP standalone + ctest" --wait 180 -- bash -c '
...
bash scripts/loop_lock.sh run agent-upstream --purpose "item1 spike: irx_differential harness build" --wait 180 -- bash -c '
