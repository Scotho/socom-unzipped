...
- **A second GS backend, paraLLEl-GS, is the default.** `ps2xRuntime/CMakeLists.txt` adds `PS2X_GS_BACKEND` with the
  ...
  `ps2xRuntime/cmake/ParallelGS.cmake` fetches `https://github.com/Arntzen-Software/parallel-gs.git` at
...
- **The differential test.** `ps2xTest/src/ps2_vu1_program_tests.cpp` (+330) holds one case, "compiled routines match
  ...
  `third_party/ps2recomp/ps2xTest/src/vu1_native_tests.cpp` (5 `tc.Run(` cases; C24's `grep -c 'Run('` counts 6, one a comment); not compared here.
