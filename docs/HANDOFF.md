...
- **Where the loop is now (2026-09-26 04:17Z, LATEST) -- Sprint 13 is CLOSED on `sprint-13` (the close-out commit); the PR to `main` and the tag `v0.13.0` follow it.** Read `docs/CURRENT_SPRINT.md`'s "Sprint 13 -- CLOSED" block and the plan's Outcome (`docs/superpowers/plans/2026-09-25-sprint-13.md`). The next controller opens Sprint 14 ("guards, not sentences", its pair in `docs/superpowers/`) off `main` after the merge; its Task 0 commits the Sprint 14 and 15 pairs and the never-run cloud handoff. Three issues wait on the owner (HUMAN_TASKS O16).
