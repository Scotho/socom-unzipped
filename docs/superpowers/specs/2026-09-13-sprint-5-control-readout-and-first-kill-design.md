...
- **Round-boundary detection parameters** *(R63, registered pre-match as instrument parameters, not bars)*: a round boundary is the `mp_round_count` step ±0.5 s host to the clock restart + 1.25 s, at most 10 s long; a stall inside that span is the boundary, not a freeze (blind: a genuine freeze that begins exactly on a round step reads as the boundary). A second intact death of the same victim in one round with no round step between is always `NO-DATA double-death`.
