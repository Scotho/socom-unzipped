...
- **The seed tasks from R257–R263**, in this order, each with its code half and its proof half:
  ...
  2. **Task 7c — vtable slots through RTTI — is the LOCAL controller's, dispatched under R260 the moment 7b landed.
     Do not implement it; do not edit `tools_py/ghidra_symbol_match.py` until the local controller says 7c is
     merged.** Consume its proposals file when it lands (merge `origin/sprint-11` into `sprint-12`).
...
6. **Vtable coverage after 7c** (consume, do not redo): the classes 7c could not open (66 names absent in retail,
...
- **Branch.** `sprint-12` opens off `origin/sprint-11` (not off `main`: Sprint 11 is still open and carries Task 7,
  ...
  `sprint-12` at least daily and whenever the local controller reports a landing (7c, the r0004 rows); when
...
- **Files you never edit on `sprint-12`** (they are Sprint 11's live state and would conflict at every merge):
  ...
  Sprint 11 owns, `tools_py/ghidra_symbol_match.py` until 7c has merged. Your live state lives in the Sprint 12
...
> the machine, Sprint 11 and Task 7c; you own Sprint 12 on branch `sprint-12`. Proceed without waiting on anyone;
