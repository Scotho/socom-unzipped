...
- **Line numbers** are the numbers at `5103637` (the branch tip when this was written). Before starting a task, `git diff 5103637 -- <the task's files>` and reconcile, saying so in the ledger. Every edit below also names the **text** it replaces, so a moved line is found by `grep -an`.
