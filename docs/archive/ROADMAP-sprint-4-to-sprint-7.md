...
> **ARCHIVED 2026-09-22.** This was `docs/ROADMAP.md` from 2026-09-12 until 2026-09-22. It is the Sprint 4 report
> with supersession blockquotes layered on it through Sprint 7's close-out (`df65444`, 2026-09-18), and it was never
> updated for Sprints 8, 9 or 10. It is kept **verbatim** because fifteen documents cite it by section — every
> `ROADMAP.md §3 / §6 / §7` reference written before 2026-09-22 means *this file*, and its narrative of what
> overturned what (§3) and its findings are still worth reading as history.
>
> **Do not plan from it.** The live roadmap is `docs/ROADMAP.md` (rewritten 2026-09-22, keeping the same §1–§7
> skeleton), and its §0 is a claim-by-claim audit of this document against the tree: what still holds, what was
> superseded, and the two entries that were simply wrong. The live state is `docs/STATUS.md` (Current state),
> `docs/KNOWN.md` (proven vs believed) and `docs/CURRENT_SPRINT.md` (what is being worked).
...
5. **Gameplay-state probe as the gate's first correctness leg** — root-node Y, a rand-derived field, MoveScale `+0x1368`, the heading matrix and a teleport count against console numbers; closes the gate-evidence gap (every mission gate from 2026-09-12 to `8281254` proved at most "the mission loaded").
   - **Added 2026-09-14 (owner-agreed):** a **console-vs-ours image comparison at fixed gameplay moments**, from PCSX2 savestates (slot 8 = Seeding Chaos spawn) — the gate compares our runs only with our own earlier runs, so a defect present in every run (the grey water shards, visible since Sprint 3) scores as perfect agreement.
   - **Added 2026-09-14 (owner-agreed):** the mission stage **fails outright on a MISSION FAILURE or "leaving designated mission area" screen** — the close-out gate `s5_head_1x_b` passed while its holds walked the player out of the mission (right-stick turn teleport) and the failure screen counted as gameplay by the letterbox band test.
