...
- **R167** (Task 8): **Linux is proven by CI, the VM's C++ and Python suites and `test_knobs_line` on the Linux runner; the VM's title stage is read for its log, not its score.** R107b and R148 already record that the VM's gate cannot discriminate at 1.6 fps. *Cost if wrong:* a Linux-only break in `drive.py`'s direct launch — which Step 3 exists to catch when the VM is free.
