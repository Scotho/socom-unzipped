...
- **R139** (the owner's request, 2026-09-19; recorded here 2026-09-25 by Sprint 13 Task R3 -- it was made between Goal 1's plan and this one, in commit `c64373a`'s message, and written into no plan's rulings; the text below is the commit's): **a crouch shortcut.** Stance in SOCOM II is hard-coded on Triangle and acts on pressure -- a peak under 0.3 toggles crouch at release, 0.3 or more goes prone -- and our pad HLE reported full pressure for every pressed button, so a PC pad could only go prone. The chosen host control (`PS2X_PAD_CROUCH_SHORTCUT`: the left-stick click, the touchpad or L2) sends Triangle at 0x40 instead of its own PS2 button; a full Triangle from any source outranks it; off sends no variable and is the runtime as before. The launcher's default became the stick click by the owner's choice on 2026-09-20.
...
- **R151** (Task 5, controller 2026-09-20): **the release executable keeps `-O1` for the generated code; `-Os`, and both LTO scopes, are not built.** Measured: M1 (`-O2`) made the exe 9.9% smaller on disk and the ZIP 4.6 MB LARGER (62,787,774 against the developer's closure-only 58,221,191) -- the download is the compressed figure and `-O2`'s code deflates worse; it also costs 1,640 s of wall time, one unit alone 1,546 s. M3 (`--icf=all` on M1's objects, a 32 s relink) bought 0.2 MB (62,585,157). The candidate that won is `REL_GENOPT=-O1 REL_ICF=all` with the release link hygiene and the strip: exe 226,286,080, **zip 55,682,325 (-15.0% against the 65,494,194 this goal started from, -4.4% against the closure-only developer pack)**, built in 733 s, 16 needed / 0 missing / 0 orphans, **gate 3/3 `s9_g2_release_gate` on that exact file (sha256 d6e30021...2bfd, title 19/23 as every gate)**. `-Os` is skipped because the only lever left is compressed size and the one data point says denser optimisation compresses worse, at 25 minutes a try; LTO over everything is the case the 30-minute stop rule exists for and inlines in the direction that lost; LTO over the runtime alone is bounded at 1.3% of the exe (R142). Launches spent: one of the three budgeted. *Cost if wrong:* a few percent of download left on the table; `REL_GENOPT=-Os ./build.sh release` is one command for whoever wants the number.
