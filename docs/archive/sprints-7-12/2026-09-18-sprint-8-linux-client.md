...
- **R106** (2026-09-18, Task 8): the audio bar (the intro correlating with the disc at 0.99) is read from the gate's driven title stage in the VM (Task 10), not from the bare boot Task 8 planned: an undriven boot sits on the controller prompt with no music for as long as it runs (a 100 s boot dumped 83 s of silence, `logs/parity/vm/boot100.wav`), because the intro only starts on a button. The frame bar stays on the bare boot (it is the same prompt on both sides, 0.008). *Cost if wrong:* none to the number; the title stage's dump is the one Sprint 7's title bar was read from on Windows too.

- **R107** (2026-09-18, Task 8/10; *issued twice on 2026-09-18: the menu-render-cost plan issued R107 first, at 11:26 in `9b55ea9`, and this one followed at 12:19 in `b944cd4`; this, the second, is cited as R107b from 2026-09-25*): the Linux audio bar (the title music correlating with the disc at 0.99) is not readable in the VM and moves to the owner's real-GPU run (HUMAN_TASKS). The passing dumped title stage (`s8_frz_A`, 19/23) dumps 244 s of silence with `pcm_underruns=22998` on the audio trace: at the software rasteriser's 1.6 fps the guest runs at a third of wall time and fills the music ring late nearly every block, and Sprint 7's once-per-fill policy (R97) silences a late fill by design. The audio path on Linux is proven by the mixer suite (554/554 under ASan, the ring, VPK and VAG cases included), by the dump's mechanics (the callback renders at 96-100% of wall, max_render 0.26 ms), and by the same exe producing the same frame; the number waits for 60 fps. *Cost if wrong:* a Linux-only audio defect that shows only at full speed goes unmeasured until the owner's run; the dump and the correlation tool are ready for that run's log.
