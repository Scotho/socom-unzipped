...
## Task 1 — Split `transfer=`, and ask whether the bytes repeat (R108b's named experiment)
...
//
// Goal 2's measurement (R108b) found the login screen pushing ~8000 host->local transfers a second
...
Sprint 8 Goal 2b Task 1. Goal 2 stopped itself at R108b: on the login screen the GL side is already
...
- [ ] **Step 7: The stop rule — apply it before Task 2, in writing.** Carried forward from spec §3 and R108b, in this goal's terms:
...
## Task 2 — An identical upload does nothing (R108b's "skip the swizzle for tiles whose bytes did not change")
...
- **R122** (Task 2, owner/controller 2026-09-19 -- **IMPLEMENTED, MEASURED, SUPERSEDED BY R123**: it fired on 4% of uploads and its per-upload invalidation sweep cost ~84 ms/s, more than it saved; **the upload-path skip is REMOVED and the instrument stays**): **the identity cache's validity is decided by rectangle OVERLAP, not by a shadow-generation stamp.** R118's first implementation guarded a remembered hash with the maximum `m_shadowPageGeneration` over the destination's page span. `markShadowPages` (`gs_gl_backend.cpp:1843-1848`) stamps that span from the buffer's *base* page on every upload, so every sibling tile written into the same atlas re-stamped every other tile's pages: in a measured 1,112,392 identical-byte uploads the skip fired **zero** times, 91.6% refused by the generation clause and 8.4% by R118's render-target guard (`logs/run_A_20260919_072444.log`). Replaced by *destination rectangle -> hash + length + 96-byte content sample*, valid until a write of **different** content overlaps that rectangle, bucketed by VRAM page row, rect-exact within a buffer and page-wholesale across buffers, invalidated from an audited list of every GL-backend shadow writer (there is no observer on `GSMem`: it is a namespace of free functions over a raw `uint8_t*`; `GSGlBackend` is the thinnest layer every shadow write passes through). Measured with `shadowSuspectForPages` corrected from the render target's widest extent (320 of 512 pages, permanently true with two targets) to its used extent, and with `m_gpuDirtyPages` dropped from the guard (it is the game thread's mirror for guest readbacks, cleared only by `syncDirtyPagesForRead`, so on a menu it is set once and stands forever): the skip then fired on 89,100 of 2.27 M login-screen uploads (4.0%) with R118 legitimately refusing 84% of the identical ones, and moved nothing -- `decodes/s` 551 vs 552, `invalidations/s` 551 vs 552, `transfer=` 91.3 vs 92.5 ms/s, fps 54.3 vs 54.2 (`logs/run_A_20260919_075140.log` against `_075449.log`, same binary, `PS2X_GS_NO_UPLOAD_SKIP=1`). *Cost of having tried it:* two launches and the knowledge that the upload side cannot pay, which is what R123 rests on.

- **R123** (Task 2, controller 2026-09-19, after two honest negatives on the upload side): **the fix moves to the consumer.** The cost was never the upload. It is `resolveTexture`'s gate (`gs_gl_backend.cpp:3209-3222`) deleting and re-decoding a cached texture because a *generation stamp* moved, not because the *content* did: 551-1385 decodes a second, every one replacing a live entry, against 26-52 cached textures -- the whole cache, every frame. Each `TextureEntry` now carries `sourceHash`, a 64-bit FNV-1a over **the exact source bytes the decode read**, in `decodeTexture`'s own order: `GSMem::ReadSpan(psm, shadow, tbp0, tbw, 0, y, width, row)` for `y` in `[0, height)`, each row's `width` texel values left to right, with `width`/`height`/`psm`/`tbw` mixed in; then, for an indexed format, the **256 CLUT entries the decode resolved** -- through `resolveClutIndex`, so `csa`/`csm`/`cou`/`cov` are already applied, and a palette write outside this texture's window is never read by its decode and must not force one. On the stale-generation branch the same walk is taken over the *current* shadow; on a match the entry's generation is restamped and the same GL texture id is returned (`revalidated`), on a mismatch or an unhashable format everything proceeds exactly as today (`decoded`). **Nothing about uploads, `markShadowPages`, R118 or render targets changes.** *Why the hashed shadow bytes are the bytes a decode would read at that moment:* `resolveTexture` does its render-target reconciliation **before** the cache gate -- the RT-as-texture fast path returns first, then the loop over `m_renderTargets` calls `downloadRenderTargetToShadow` + `markShadowPages` for every `shadowStale` target overlapping the texture's pages -- so any pending GPU-to-shadow download has already been performed when the gate is reached; that download is also what moved the generation, and it changes the bytes, so such a texture re-hashes differently and is decoded rather than revalidated. `PS2X_GS_NO_TEX_REVALIDATE=1` restores today's behaviour for the A/B. Full hash for every size to begin with, measured before any cutoff is added. *Cost if wrong:* a stale texture on screen where the content changed but the hash did not -- a 64-bit collision over the whole source -- with the knob as the one-run bisect and the three-stage gate as the guard.

- **R124** (controller 2026-09-19, recorded here 2026-09-25 by Sprint 13 Task R3 -- it was made in commit `4c34a97`'s message and never written into this list, which is why the 2026-09-25 documents audit, D56, found no text for it; the text below is the commit's): **a hard ceiling on pending GS command bytes.** State-carrying commands can never be dropped, so above `PS2X_GS_PENDING_HARD_CAP_MB` (default 1024 MB, 0 = none) `record()` waits on the queue's condition variable in 50 ms slices, 5 s at most per command, instead of growing until `std::bad_alloc`; both swap sites notify after releasing bytes, and `hard_waits=` joins the back-pressure stats line. *Why:* the 8 GB VM died of `std::bad_alloc` on a long stall, and this is that wall. The wait loop itself has no test (the test process has no GL context). Sprint 10 Q6's latched-stall bound keeps it as the last line for an unlatched-but-slow replay.

- **R125** (Task 4, controller 2026-09-19): **the goal closes on the symptom, not on the 60 ms/s proxy.** After R123 the loaded login run (`logs/s8_menu_bar2.sh`, four spinning cores, 60 s hold, `logs/run_A_20260919_084912.log`) shows 58-60 presents a second in every one of its last 60 rows, `bp_pending` at most 2 with no waiter, `pcm_underruns` 0, `transfer=` 20.9 ms/s. `upload=` is 142 ms/s, so upload+transfer is ~163 against the bar of 60, and what is left is the shadow swizzle of ~8,200 uploads a second (9.7 us each), which the upload-side skip could not remove (R122: R118 rightly refuses 84% of the identical ones). The ms/s bar was chosen as a stand-in for "the login screen holds 60 fps under host load"; that is now measured directly and holds. Task 3 is not built (branch B1: decode was the flush and Task 2 removed it). *Cost if wrong:* the 4-in-10 slow launch was a rate and this is one loaded launch; KNOWN's row says a recurrence reopens it, and the knob `PS2X_GS_NO_TEX_REVALIDATE=1` is the one-run A/B.
