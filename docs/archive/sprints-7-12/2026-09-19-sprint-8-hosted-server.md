...
- Rulings on the owner's behalf are numbered from **R109**. *(2026-09-25: the menu-render-cost plan had already issued R109 and R110, so this plan's two are cited as R109b and R110b; the notes are on the rulings themselves.)*
...
- [x] Message d1: the address, and the preset's note text (`"the project's hosted server (us-east)"`). d1 changes `kServerPresets[1]` in `launcher_config.h`, the default `serverPreset` to `"unzipped"` per the Sprint 7 spec, and the test in `launcher_tests.cpp` (the preset's address is not a `_TBC` placeholder and parses as an IP or hostname; `effectiveServer` of a default config is that address), after its launcher commit lands.
- [x] Proof: the launcher's `--screenshot` of the ONLINE page showing the preset and the address (d1's tool, d1's run). `docs/HUMAN_TASKS.md`: the "two server addresses" item gets a dated line — ours is supplied; the community's stays the owner's (and Goal 10's).

## Task 6 — a match on it (bar 4; in d1's queue)

- [x] Message d1 the address; it runs, under the lock with builders held: (1) the control round (`scripts/parity/online_control_round.sh`-shape, `SOCOM_SERVER_IP=<ip>`), (2) the kill round (the ladder). Expected evidence per round: both instances' lobby reached, game created and joined, the round's start frame; for (2) the kill line.
- [x] While each runs, this session watches the box: `journalctl -f` for the four units, `ss -lun | grep 500` for the per-client DME sockets, the advertised address in each reply. Saved to `logs/parity/<stamp>/server-side.log`.
- [x] Verdict written where the round's is: round started Y/N, kill Y/N, and what the hosted path exercised for the first time (both clients behind one NAT — the hairpin case; the clock skew between the host and the box). Findings to `docs/KNOWN.md` **through d1** (one writer).
- [x] Stop rule (spec): no round start here but one on the LAN server with the same exe → stop at the packet-level difference and file it.
...
- **R109 (2026-09-19).** *(Issued twice: the menu-render-cost plan issued R109 on 2026-09-18 in `9b55ea9`, and this plan reused it on 2026-09-19 in `9bf44a9`; this, the second, is cited as R109b from 2026-09-25.)* The launcher's default preset moves from `custom` to `unzipped` now that 3.143.65.100 is real — why: the Sprint 7 spec said the default switches "when ours is real", and a stranger's first run should reach a lobby without typing an address; d1 added a guard so a `config.json` from before the picker (a typed `server`, no `serverPreset`) stays `custom` with the player's own address — cost if wrong: a first login goes to a server that is down or gone when the credit runs out (2027-03-05 at the latest); the ONLINE page still offers Custom.
- **R110 (2026-09-19).** *(Issued twice: the menu-render-cost plan issued R110 on 2026-09-18 in `9b55ea9`, and this plan reused it on 2026-09-19 in `9bf44a9`; this, the second, is cited as R110b from 2026-09-25.)* The static IP ships in the launcher rather than a DNS name — why: no domain is owned and buying one is the owner's money; `horizon-ctl.sh public-ip` and the preset both take a hostname, so moving is one line each — cost if wrong: a change of host means a new launcher build for every player.
...
- **Finding, 2026-09-19 06:03 UTC (the first driven round, `s8_hosted_control`).** Ubuntu's unattended upgrade ran `needrestart` and restarted all four Horizon units in the middle of the round. It did not cause that round's failure (LOBBY-FAIL at the login form, the harness's persona path -- d1's to settle) but it would drop every connected player. Fixed on the box and in `install.sh`: `/etc/needrestart/conf.d/50-horizon.conf` excludes `horizon-*`; the units restart by hand only. The same capture is the first evidence of a real SOCOM II client on the hosted server: MUIS universe news, the MAS RT handshake and `MediusSessionBegin` from <the owner's address>, plus one unhandled `MessageClassLobby MsgType:134` per session.
- **Finding, 2026-09-19 (d1's A/B, `s8_lan_login_check` vs `s8_hosted_control`).** The game keeps saved personas per server, and with configs byte-identical apart from the address the key is evidently the server address. Consequences: (1) every player's first login on the hosted server is a create-persona login (name keyboard, then password; `CreateAccountOnNotFound=True` makes the account) -- the harness had no such path and d1 is adding one to `online_login_ours.py`; (2) **moving the server's address orphans every player's saved persona**, which raises the cost-if-wrong of R110b: the static IP `socom-unzipped-ip` must not be released, and a move to a DNS name is itself such a move (whether the game keys on the string it was given or the resolved IP is untested) -- decide it before strangers have personas, not after.
- **Tasks 5-6 results (2026-09-19), GOAL MET.** (5) The preset is real and the default (d1, `aa2b7f4`): `logs/parity/launcher_ui/online_1100x700.png` shows *SOCOM Unzipped (project server)* selected, "the project's hosted server (US East)", ADDRESS 3.143.65.100. (6a) `s8_hosted_control` LOBBY-FAIL at the login form -- the harness had no first-time (create-persona) login; fixed by d1 in `ff514f7`. (6b) `s8_hosted_control2` **PASS**: `CONTROL-ROUND round_ended=yes`, both instances in gameplay on Frostfire for the whole five-minute round, clocks in step; the box saw 2 AccountLogin, 1 CreateGame, 2 JoinGame, two 50000+ sockets bound for the round and released after it, no restart. (6c) `s8_hosted_kill` **PASS**: `LADDER-SUMMARY rounds=4/4 usable=4 best_rung=3 kills=2`, round 4 `signal=health on=B` (1.0 -> 0.0), zero starvation alarms, max NetIdle 1.3-1.4 s; the summary's `RUNG0 FAIL back-pressure waits` is the host's render back-pressure (Goal 2's ground), not the network. Harness `ff514f7fa972`, exe `b966050ee35c36d2`. Two clients behind one NAT against a remote Horizon work; the hairpin worry (peers told each other's public address) did not bite -- game data relays through the DME. The box after the four rounds: 537 MB resident across the four processes (335 MB idle), 1.1 GB available, load 0.03.
- **Left for the owner:** their own match from the launcher (HUMAN_TASKS, text handed to d1); the domain-name decision before strangers have personas (the finding above); the credit re-read against $12/month; an IAM user instead of root sign-in.
