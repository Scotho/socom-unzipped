> **ARCHIVED 2026-09-26 (Sprint 14 Task I2) -- superseded by the four skills** `.claude/skills/loop-iteration/`,
> `agent-worktree/`, `run-gate/` and `sprint-close/` (each a `SKILL.md`); `docs/LOOP_PROMPT.md` is now a pointer to
> them. Below is `docs/LOOP_PROMPT.md` verbatim as it stood at `521edbc2`, before the cut. **Nothing below is an
> instruction:** every "Every iteration" step, "Lock protocol" paragraph and "LOOP_PROMPT step N" cited before
> 2026-09-26 means this file; the live procedure is the skill.
