# HANDOFF.md -- the controller-to-controller file as it stood before it became transient (archived 2026-09-26)

> **ARCHIVED 2026-09-26 (Sprint 14 I3) -- superseded by the transient `docs/HANDOFF.md`; every citation of a HANDOFF
> section written before this day means this file** ("HANDOFF §5 rule 9", "trap 5", "§7", "§8"). The live file does
> one job now -- what is in flight, what is owed, what to do first, under 6,000 bytes -- and the durable content moved
> to its proper home: §1's map and the guards to `CLAUDE.md`; §3 to the live file's §3, five lines naming the
> `loop-iteration` skill; §5's rules to one line each in the live file's §4, same numbers, each naming its guard or its
> home -- **their reasons stay here, in §5 below, by rule number**; §5's "Giving an agent a worktree" to the
> `agent-worktree` skill (`.claude/skills/`, beside `run-gate` and `sprint-close`); §6's traps to `docs/HAZARDS.md`
> under the area each bites (traps 1, 2 and 9-16, each tagged "from HANDOFF §6 trap N"; traps 3-8 were already
> hazards there and stay only here); §7's instruments to `docs/DEVELOPING.md` "Instruments and diagnostics",
> verbatim; §8 to the live file's §5, rewritten as it is now; §11's document classes to `docs/DOC_MAINTENANCE.md` §3,
> which already owned them; what is true to `docs/KNOWN.md`. §2's older pick-up points and §4, §9 and §10 were
> already in `docs/archive/HANDOFF-loop-history-to-2026-09-25.md`. Below the rule, the file exactly as it was
> committed on `sprint-14` at `696d0618`; nothing in it is an instruction.
...
- **Next free ruling number: R279.** (It read **R179** from 2026-09-20 to 2026-09-22 while R240 was in use -- and a
