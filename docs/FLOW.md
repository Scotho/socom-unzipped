...
Rendered at commit `cfe840059813a48256519540c1304285e46c35e6` (committer time 2026-09-26T10:20:25Z), since 2026-09-01 (UTC days): 1803 commits, 149 merges.

## The numbers

- **merges per day**: 149 merges on 9 days (mean 16.6 a merge day); `git log --merges` counts 149. Per day below -- command: `python -m tools_py.changelog --rev cfe84005` (its merges, changelog.entries()), by UTC committer day; cross-check: `TZ=UTC git log --merges --since=2026-09-01T00:00:00Z --date=format-local:%F --format=%cd cfe84005 | sort | uniq -c`
...
- **subjects saying " again"**: 45 of 1803 commits -- command: `git log --since=2026-09-01T00:00:00Z --format=%s cfe84005 | grep -ci -- ' again'`
- **docs share of churn, seven days**: 30.9 %: 128997 of 417723 lines added + deleted, 2026-09-19T10:20:25Z to 2026-09-26T10:20:25Z -- command: `git log --numstat --no-renames --since=2026-09-19T10:20:25Z --until=2026-09-26T10:20:25Z --format= cfe84005 | awk '$1 ~ /^[0-9]+$/ {t += $1 + $2} $3 ~ /^docs\// {d += $1 + $2} END {print d, t}'`
- **ticket wait**: not measured: the queue log was not found. Task W2 makes the lock write a `TICKET <id> waited <s>` line into logs/loop_lock.queue.log at each grant; until then there is nothing to read -- command: `grep -E 'TICKET [^ ]+ waited [0-9]+' logs/loop_lock.queue.log` (the main tree's), the median of the last field, unstamped lines and lines stamped after 2026-09-26T10:20:25Z or before 2026-09-01 left out
- **open-issue age**: not measured: the open-issues table of docs/BACKLOG.md at `cfe84005` has no Opened column; the dates live on GitHub (`gh issue list --state open --json number,createdAt`), a network read this page does not make -- command: `git show cfe84005:docs/BACKLOG.md`, the Opened column of section 1's table, days to 2026-09-26
- **sessions**: 18 distinct Claude-Session trailers -- command: `git log --since=2026-09-01T00:00:00Z --format=%B cfe84005 | grep '^Claude-Session:' | sort -u | wc -l`
- **commits per session**: largest first: 333, 220, 136, 85, 61, 32, 30, 21, 16, 12, 10, 9, 8, 7, 3, 3, 2, 1; median 14 -- command: `git log --since=2026-09-01T00:00:00Z --format=%B cfe84005 | grep '^Claude-Session:' | sort | uniq -c | sort -rn`
