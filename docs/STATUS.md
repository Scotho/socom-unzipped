...
- **2026-09-18 (afternoon):** branch `sprint-8`, Sprint 7 merged into develop and main (`b65fe46`). Sprint 8 Goal 1, the Linux client, is built, unit-tested and booting: the Linux CI job is green end to end (compile on ubuntu-24.04, Python 1205, C++ 554), the VirtualBox VM `socom-linux` boots the game under X on Mesa GL 4.1 with the same frame as Windows (0.008, bar 3), the tarball self-tests unpacked from a fresh directory; the gate's title stage in the VM and the launcher's real run from the tarball are the last two steps, then the close-out. A double free in the mixer that only glibc reported is fixed on both platforms. Sprint 8's other goals (the menus' render cost, voice, audio residuals, window policy, bare-run robustness, knob retirement) follow. Windows: `./build.sh test` 552, gate 3/3 (`s8_mixer_gate`).
