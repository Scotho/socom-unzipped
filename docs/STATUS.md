...
- **2026-09-20 (later):** Sprint 9 Goal 2 done -- the portable download is 55.7 MB on Windows (-15%) and 99.4 MB on Linux (-13%), each exactly its import closure with a `SHA256SUMS`, the release exe stripped with symbols kept, gate 3/3 on the release exe itself (`s9_g2_release_gate`); `-O2` compressed worse and was not shipped (R151). A crouch shortcut landed (R139: the game reads Triangle's pressure; a pad could never crouch). Goals 7 (the server by name) and 8 (the launcher's bug report section over the site's endpoints) are in the spec. C++ 649/0, Python 1360.
