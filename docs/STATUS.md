...
- **What a stranger still lacks** (2026-09-20; the audit's §1 list is all done except the two-machine match): music that is coherent (Goal 10), a launcher that lets go of the pad while the game runs (Goal 9), a server reached by name (Goal 7), a first two-machine match over the internet (**owner**; every online result so far is two instances on one host), and -- before the repository is public -- licences, a history audit and install instructions (Sprint 11).
