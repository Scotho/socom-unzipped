...
- **Gate hardening.** `drive.py` captures `w{i:02d}_{k:03d}.png` every second during settle waits (not just during scripted bursts); `black_rows.py` counts them alongside `s*.png`; the transition floor is back to 5 frames (calibration runs `wcap1`/`wcap2` examined 10 and 13). `tests/fixtures/gate/{title,transition,mission}` are committed (title 16 frames, transition 5, mission good/bad `.drive.txt`), so `python -m unittest tools_py.tests.test_gate` (17 tests) runs its positive cases on a fresh clone with no prior run needed. `PS2X_TEST_REPEAT=N ./build.sh test` runs the unit suite N times (5/5 green; no flake reproduced) (Tasks 8-10).
