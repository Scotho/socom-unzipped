...
*The project's timeline, first commit to today. Written for people who want to play the game, not build it. The
technical bit of each entry is one line and skippable; the rest is what happened.*
...
what was retracted, what a number doesn't mean. `Cited:` is the evidence: commit hashes you can open in the repository,
and run records. The run records live in a `logs/` folder on the project's own machine, not in the repository, so each
one cited here is backed by a frozen witness (first lines, size, hash) in `docs/story/witnesses.json`. A test fails if
any citation stops pointing at something real.

**Dates** are the commits'. A few documents in the repo are stamped a day later than the commit they describe, because
...
*But:* compiling isn't running. Forcing entry points by hand also pushed the count of untranslatable instructions from about 11,000 to 114,000 — garbage that never executes, still there today.
...
The stand-in for the PS2's movie library never cleared its work buffer when a player was created. The game read the first word of that buffer to ask whether the movie had finished, found leftover bytes spelling UIMe from an unrelated file, and concluded it had already ended. The real function clears the buffer. With that one line — and the game's feed callback run on the calling thread instead of parking the only thread that could feed the decoder — the Sony logo, the intro and the looping menu background all play. The main menu's score went from 81.0 to 98.8.
...
*But:* a test count isn't correctness and neither is a green gate. The gate proves regression — the game looks like it did last time — and one of its three stages turned out to be measuring nothing at all. Next entry.
...
*But:* nothing caught either of these for weeks. The parity gate was green over them the whole time — a defect present in every run looks exactly like the reference. The maths routines were milder than billed: 19 of their 22 call sites happened to want the identity anyway.
...
*How:* two independent readers — one watching the actor's health field, the other replaying the round's own state values — had to agree before a round counted.
...
*A player with their own disc can unzip a 56 MB download, start the game on Windows or Linux, save to a memory card, report a bug from inside it, and play online against a server the project hosts in Ohio — and as of tonight, against a console player in the same lobby. Only the owner's PC has done any of it.*
...
*But:* the download is unsigned, so Windows warns anyone who runs it. Two promises — that a double-clicked exe lets go of its console window, and that a missing audio device is reported on screen — are believed, not proven.
...
*But:* exactly one real report has ever been sent, from Windows. The project's own notes briefly recorded it as unevidenced — the id is stored lower-case and shown upper-case, and the inbox mirror was 35 minutes stale. Found the next day.
...
*But:* the packaging failure had a shape worth remembering: the build script had already emptied the folder it was about to refill, the packaging step then refused, and the previous archive — twelve hours old, plausible in every way — stayed exactly where it was. A tag on that folder rather than on the run would have shipped it with no symptom.
...
The open question is still the one the owner found on the first evening: the music. The device buffer, the pan, the
silent voice lines and the missing ambience bed are all fixed and measured; the still-playing answer that let stems
chain on the console is fixed and being verified; the bed is still 7-12 dB quieter than the console's and the logo
movies are quiet at the source. The owner's ear decides when it's done, and the last verdict was "better, not done".
