...
| **Equal-priority time slicing is an ours-only semantic that could corrupt shared structures online** (`EeScheduler` expires a 65536-cycle slice when an equal-priority thread is ready; the PS2 kernel never time-slices; audit §2.3). | Log SOCOM's thread priorities once; expire the slice only for strictly higher priority under a test; then a control round (Sprint 7, before shipping online). |
| **`sceCdRead` advances the CD stream cursor** (`CD.cpp:328`), so a plain read while a stream is open feeds the demux from after that file; unobserved, may be the 0.26 s slip at 120 s in research/32 §7.1. | A separate stream cursor and the test 'StRead after Read resumes at the stream LBN' (Sprint 7). |
