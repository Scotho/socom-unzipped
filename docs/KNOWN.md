...
| **On Linux, the driven title stage freezes on one frame when PS2X_AUDIO_DUMP is set** (two runs, every capture the same frame, scores 60.2 and 59.1; the dump grows to ~55 MB of near-silence) and passes without it (`s8_vm_title5`, 19/23); on Windows the dump has run in dozens of gates. The dump is an fwrite under a mutex on the audio callback thread; the VM's virtual disk and PulseAudio are the suspects | `s8_vm_title_audio2`, `s8_vm_title_audio3` in the VM (2026-09-18 16:30-17:00). Experiment: the same run with the dump on tmpfs (/dev/shm), with the stream worker off, with the trace alone; the freeze tracer on the run log names the thread. Sprint 8 Goal 1 Task 8's audio bar waits on it (R106) |
