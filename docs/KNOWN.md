...
| **The lobby rate on a pinned harness is 6/10 (bar 8/10) and every miss is one class: the on-screen keyboard at login types extra characters** (`12 of 4`, `8 of 5`, `9 of 5` characters left in the field after the drive's clearing; once the ENTER left the keyboard up) -- the class predates the scheduler change (drive files of 2026-09-16 22:32 and 2026-09-17 00:17 carry it) | `logs/parity/lobby_rate_summary.txt` 2026-09-17 22:28, harness `f57a424`, exe `85c36119`; `lobby_report --bar 0.8`: `RATE 6/10 FAIL classes: login:keyboard-typing=4`. Sprint 7 Task 2f owns it |
...
| **21k texture uploads a second in an online round even with stable CLUT ids** (45 ms/s of render time; was 64k/s with the cache exploding). Textures whose pages take a shadow-generation bump every frame are re-decoded — the palette rewrites likely share a page with a texture. | `PS2X_GS_STATS=1` plus the `[gs-pages]` trace on the HUD/skin texture pages; the cache key's page span. |
...
| **The login screen runs at 12-30 fps under GL back-pressure in 4 of 10 launches** (`bp_pending=4 bp_waiters=1`, `bp_wait_ms` growing 5-52 s over the screen, the guest clock 2.8 s behind wall, one >250 ms frame; the other 6 launches sit at 59-60 fps with `bp_wait_ms` growing 130-233 ms over 25 s). A 2D menu should never wait on the GL thread; whatever the GL thread is doing there is slow on this machine and will be slower on a stranger's. The driven lobby misses are its symptom (the drive's 90 ms wall-clock press falls between two 80 ms frames). | `logs/run_A_20260917_225536.log` 269-280 s vs `run_A_20260917_222858.log` 215-225 s (pc-sampler lines). Experiment: `PS2X_GS_STATS=1` over the login screen of a slow launch, the stats line's textures=/decodes and pending frames per second against a fast one; then Task 3's page trace on the login screen's pages. Sprint 7 Task 2f latches the injected press (the harness half); the slowness is its own row until measured. |
| **The 21k texture decodes a second come from over-wide cache invalidation**: page marks are a global generation bump at page-row granularity with no x-extent, and a render target overlapping a sampled texture is downloaded and its whole span re-stamped every frame (audit §2.2 F1; the post-process draws into the depth-buffer pages every frame, which overlap the parked streamed textures). | `PS2X_GS_TRACE_PAGES` on a HUD texture page: download lines preceding decode lines each frame. Then mark only downloaded rows and add a decodes-per-present budget to the console-replay GL test (Sprint 7). |
