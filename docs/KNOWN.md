...
|---|---|
| **Requiem: the joiner fails the control bar on both passes** (research/33: B's holds net 13–38 u against 40, A passes; B's guest clock 0.38 s per wall second, the sweep's slowest, A 0.57). B moves — the bar is measured in host seconds against a game clock running at a third of wall speed. Believed the slow-clock item above, not a Requiem defect. | A Requiem launch with `PS2X_VU_STATS=1 PS2X_GS_STATS=1` on B; or the control bar re-expressed in guest seconds (`0x4365c0` delta per hold). |
| **Foxhunt: the control round's negative control fails on fall damage** (research/33: B strafed off a 110-unit drop at T+224 s, health 1.0 → 0.738, no kill, no counter step). A harness matter — the blind strafe legs do not look for ledges. | Shorter legs, or a y-drop check that neutralises the pad when the actor falls more than a body height; then a Foxhunt re-run. |
| **The guest clock runs at 0.5–0.85 of wall time even when a round plays** (research/34 §6: `DAT_004365c0` 13.8 s per 30 s on `ours_control_frostfire_bisect3`, 0.74 s/s on `_clutfix`; PCSX2 is 1:1). The VU1 interpreter's host time is excluded from guest time by design, and that exclusion is the slowdown — so an online round is longer in wall time than its clock, and anything paced on the guest clock (weapon timers, the 0.6 s snap-back) is stretched. | `PS2X_VU_STATS=1` host ms/s against `[clock]` eeCycle/host over a round; a native-VU1 run for comparison. |
