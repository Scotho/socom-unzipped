...
- ~~**The 989snd PCM ring lives at EE address 0x900000, inside the game's own memory**~~ **Closed 2026-09-17 (research/32 §7.1):** the ring is at 0x000A0000 now — below the ELF, above the runtime's kernel mirror, under the EE's 24-bit mask. The title music's scratch, chased through this row, was the sceMpeg HLE feeding the game's audio callback wrong: callbacks ran out of order (the EE dispatcher stacked queued invocations last-in first-out), late (after the demux call instead of inside it), and a packet the callback refused for lack of room was consumed anyway, shifting the whole stream by one byte. All three fixed under tests; the ring reads the movie audio's 512-byte L/R blocks.
