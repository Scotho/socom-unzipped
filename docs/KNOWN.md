...
- **The pixel-identity console-replay test has never run in the suite, and its dump is gone.** `ps2_gs_tests.cpp`'s console-replay GL case returns at once unless `PS2X_CONSOLE_REPLAY_DIR` is set, and nothing in the repo sets it; the dump it needs (`packets.bin`, `vram_initial.bin`, research/31 §12: 5386 packets, ~16 s a replay) is nowhere on this machine -- only its PNGs survive in `docs/research/assets/`. Found 2026-09-19 while looking for a pixel guard for the texture-cache change. Until a dump is captured again (a PCSX2 GS dump of a menu and a gameplay frame, the recipe is research/31's), the three-stage parity gate is the only pixel guard a GL change has.
