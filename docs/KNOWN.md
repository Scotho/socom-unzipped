...
| **Foxhunt: the control round's negative control fails on fall damage** *(guard built 2026-09-17: the driver watches the actor's height each leg; a drop of 30+ units is a fall -- pad neutral 1 s, the side's legs turn back -- and a health drop within 3 s of it is scored as fall damage, apart from the negative control; `test_control_round` 20/20. Settles on the next Foxhunt control run.)* (research/33: B strafed off a 110-unit drop at T+224 s, health 1.0 → 0.738, no kill, no counter step). A harness matter — the blind strafe legs do not look for ledges. | Shorter legs, or a y-drop check that neutralises the pad when the actor falls more than a body height; then a Foxhunt re-run. |
