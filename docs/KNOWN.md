...
- **The 989snd PCM ring lives at EE address 0x900000, inside the game's own memory** (research/32 §7). The EE masks the ring position to 24 bits and subtracts the ring address, so the ring must sit under 16 MB; the runtime's guest heap does not (0x0178cf00 here), and the module falls back to the fixed address the first cut used. The SIF DMA copies 24 KiB of PCM into that region several times a second while music plays. Nothing has visibly broken in two days of runs, and the address was in use since research/06 — but a low guest reservation for the ring is owed before anyone chases a corruption there.
