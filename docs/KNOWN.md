...
|---|---|
| **Our `sceGsExecLoadImage`/`sceGsExecStoreImage` HLE multiplies the BITBLTBUF block pointer by 8** (`Kernel/Stubs/GS.cpp` ~:641 and ~:706 compute `vram_addr * 2048 / 256`). At the single-player mission load the game parks 1.75 MB of VRAM in the motion-pack buffer as seven 0x40000 pieces (`FUN_003b1990(0x2400, …, 0x1c0000)` / `FUN_003b18d0`, `vram_addr += 0x400`); with the ×8 the seven block pointers mask to 0x2000/0/0x2000/0…, so the restore writes [6,5,6,5,6] over chunks 0–4 — the motion smear, and so the turn teleport. A block scan of our RDRAM images reproduces the pattern; the stray 0x40000 copy at ~`0x1d2b000` is the load stub's freed `guestMalloc` packet. **Independent verification was in flight when work paused** | research/25 §9. Settle: the paused verification (units at the three sibling call sites, the alias table, dumps vs console), then the two-line fix + a seven-region round-trip test in `ps2_gs_tests.cpp` (the existing single-region test is lossless either way), full gate for blast radius (the same stubs carry loading screens, movies, textures), and an offline descriptor-by-name check vs console, 48 → 0 |
| **The Seeding Chaos stream renders as grey shards because GL depth precision collapses** (`gs_gl_backend.cpp:2630` `z/2^32`, shader `:235` `aPos.z * 2.0 - 1.0` in float32 → ulp ≈ 6e-8 near −1 → GS z quantised to multiples of 128). This water is Z16S at z ≈ 1000–9000, ~70 distinct depths, so water/bed/banks tie or swap under `GL_GEQUAL`. The arithmetic and the two lines are **verified**; the console (PCSX2 slot 8) shows continuous brown water at the same camera and the water mesh covers exactly that region | research/26. Settle: branch `fix/gl-depth-precision` (`glClipControl(ZERO_TO_ONE)` or `gl_FragDepth`, with `PS2X_GS_DEPTH_LEGACY=1`), gate + vram-diff + title run-vs-run, and an A/B of s28/s29 against slot 8 — **in flight when work paused** |
