...
| `docs/RULINGS.md` | **G** | `tools_py.rulings` | Every ruling with its status (active, superseded, retracted, withdrawn, vacant) and its home, global newest first, then the `S13-R<n>` and `S12-R<n>` namespaces; held to `docmaint.ruling_records()` by `rulings --check` (exit 1 on a stale file). Regenerate when a ruling is made or its status changes. Sprint 14 D1 |
