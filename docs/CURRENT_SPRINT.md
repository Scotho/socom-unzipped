...
| R251 | "**r0004 is a real rebuild and its ELF exists**" — the package in hand and decoded, the overlays read through PINE, `build_revision.sh r0004` built the ELF (5,016,016 B, `62f4f877…`) | same rulings section; `docs/archive/CURRENT_SPRINT-sprints-9-to-11.md`, the 02:5xZ block; `docs/HUMAN_TASKS.md` D1 | stands; **done** — the r0004 gate is 3/3 (`s11_r0004_probe2`) and the build plays online on our server (`s11_r0004_round2c`). It **reopened HUMAN_TASKS D1** (the per-player pipeline is the owner's to accept) and left Task 11b moot |
